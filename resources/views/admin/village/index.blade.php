...
<div class="p-6 md:p-8">
    ...
    <div class="flex items-center space-x-3 mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Village Management</h2>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50 hidden md:table-header-group">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-3/12">Village Name</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="md:hidden block border-b border-gray-200 last:border-b-0">
                        <td class="p-4 block">
                            <div class="flex items-start justify-between">
                                <strong class="text-lg font-semibold text-gray-800">{{ $village->name }}</strong>
                                
                                {{-- Status --}}
                                <span class="px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $village->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                    {{ ucfirst($village->status) }}
                                </span>
                            </div>
                            
                            {{-- Timestamps --}}
                            <div class="mt-2 text-xs text-gray-500 space-y-1">
                                <p><span class="font-medium text-gray-700">Created:</span> {{ $village->created_at->format('Y-m-d H:i') }}</p>
                                <p><span class="font-medium text-gray-700">Updated:</span> {{ $village->updated_at->format('Y-m-d H:i') }}</p>
                            </div>

                            {{-- Actions --}}
                            <div class="mt-4 flex space-x-3 justify-end border-t border-gray-100 pt-3">
                                <a href="{{ route('admin.village.edit', $village->id) }}" class="text-blue-600 hover:text-blue-900 text-sm font-medium">Edit</a>
                                <form action="{{ route('admin.village.destroy', $village->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this village?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr class="hover:bg-gray-50 hidden md:table-row">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden lg:table-cell">{{ $village->created_at->format('Y-m-d H:i') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden lg:table-cell">{{ $village->updated_at->format('Y-m-d H:i') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                            <div class="flex space-x-2 justify-center">
                                <a href="{{ route('admin.village.edit', $village->id) }}" class="text-blue-600 hover:text-blue-900" title="Edit">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-gray-500">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No Villages Found</h3>
                            <p class="mt-1 text-sm text-gray-500">
                                Get started by adding a new village entry.
                            </p>
                            <div class="mt-6">
                                <a href="{{ route('admin.village.create') }}"
