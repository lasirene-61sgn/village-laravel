...
<div class="p-6 md:p-8">
    ...
    <div class="flex items-center space-x-3 mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">{{ $title }} RSVP Reports for "{{ $event->name }}"</h2>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-6 text-gray-600">
            ...
            <p><strong>Total {{ $title }} RSVPs:</strong> <span class="text-lg font-bold text-blue-600">{{ $rsvps->total() }}</span></p>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider print:text-black print:border print:border-black">Status</th>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider print:text-black print:border print:border-black">Note</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider print:text-black print:border print:border-black">Response Date & Time</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($rsvps as $rsvp)
                        <tr class="hover:bg-gray-50 print:bg-white">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm print:text-black print:border print:border-black">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    @if($rsvp->status == 'accepted') 
                                        bg-green-100 text-green-800 
                                    @elseif($rsvp->status == 'declined') 
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 print:text-black print:border print:border-black max-w-xs">
                                {{ $rsvp->note ?? 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 print:text-black print:border print:border-black">
                                {{ $rsvp->created_at->setTimezone('Asia/Kolkata')->format('M d, Y H:i:s') }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500 print:text-black">
                                No {{ strtolower($title) }} RSVPs found for this event.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
